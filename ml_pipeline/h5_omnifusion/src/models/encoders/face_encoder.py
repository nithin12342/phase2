...
class FaceEncoder(nn.Module):
    ...
    def __init__(
        ...
        lstm_out_dim = self.hidden_size * 2 if config.bidirectional else self.hidden_size
        
        self.seq_proj = nn.Linear(lstm_out_dim, d_model)
        
        self.summary_proj = nn.Linear(lstm_out_dim, d_model)
    ...
    def forward(
        ...
        x, (h_n, c_n) = self.lstm(x)  # (B, T, lstm_out_dim)
        
        x = self.seq_proj(x)  # (B, T, d_model)
